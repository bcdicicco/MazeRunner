...
public class StandardHighScores : MonoBehaviour
{
    ...
    public void UpdateUIText()
    {
        ...
        int backwards = _scoreManager.numberOfScores;
        for (var count = 1; count <= _scoreManager.numberOfScores; count++)
        {
            // rank
            leaderboardUIText_rank.text = leaderboardUIText_rank.text + count.ToString() + ".\n";

            // name
            leaderboardUIText_name.text = leaderboardUIText_name.text + _scoreManager.GetNameAt(backwards + 1 - count) + "\n";

            // score
            leaderboardUIText_score.text = leaderboardUIText_score.text + _scoreManager.GetScoreAt(backwards + 1 - count).ToString() + "\n";
        }
